...
/** Extrae `step` del query string de una URL de htmx. */
function stepFromUrl(url: string): StepId | undefined {
  return new URLSearchParams(url.split('?')[1] ?? '').get('step') ?? undefined;
}
 ...
 */
export function parseSession($: CheerioAPI, uuid: string, step?: StepId): MotorSession {
...
export function parseStep(
  ...
  const $ = cheerio.load(html);
  const session = parseSession($, uuid, stepId);
